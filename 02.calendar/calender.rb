...
class Calendar
  ...
  def initialize(year, month)
    @this_year = year
    @this_month = month
    exit EXIT_FAILURE if not are_params_valid?
  end
  ...
  # エラーチェック
  private def are_params_valid?()
    valid = true
    if not (1 <= @this_year.to_i && @this_year.to_i <= 9999)
      puts "year \`#{@this_year}\` not in range 1..9999"
      valid = false
    end
    if not (1 <= @this_month.to_i && @this_month.to_i <= 12)
  ...
  # カレンダー年月タイトル生成
  private def display_title
    format("     %2s月 %4s年    ", @this_month.to_s, @this_year.to_s)
  end

  # カレンダー曜日生成
  private def display_day_of_the_week
    ["日", "月", "火", "水", "木", "金", "土"].join(" ")
  end

  # カレンダー日付生成
  private def display_days
    ...
    first_day.upto(last_day).each do |date|
      if date == Date.today
        # 当日に行き当たったら表示色を反転する
        days[date.wday] = "\e[7m#{date.day.to_s}\e[0m"
      else
        days[date.wday] = date.day.to_s
      end
...
opt.on("-m VAL", "月を指定します") {|v| params[:month] = v if v != nil}
opt.on("-y VAL", "年を指定します") {|v| params[:year] = v if v != nil}
opt.parse!(ARGV)
